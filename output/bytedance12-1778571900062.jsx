...
<section className="hero-forced-form" style={{position:'relative',overflow:'hidden',background:'#05070d',color:'#fff',minHeight:'92vh',padding:'120px 24px 90px'}}>
  <video autoPlay muted loop playsInline style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',opacity:.38}}>
        <source src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/08-8c93b9a6e6.mp4" type="video/mp4" />
      </video>
...
<section className="tms" style={{background:'#111827',padding:'60px 0',position:'relative',overflow:'hidden'}}>
  <div className="tms-grid" style={{display:'flex',justifyContent:'space-around',alignItems:'center',position:'relative',zIndex:1}}>
    <div className="tms-cell reveal" style={{textAlign:'center',flex:'1'}}>
      <div className="tms-val" data-count="500" style={{fontSize:'40px',fontFamily:'JetBrains Mono',color:'#ff6b00'}}>//</div>
      <div className="tms-label" style={{fontSize:'20px',color:'#ffffff'}}>//*[@comment:* Your label goes here. *//</div>
      <div className="tms-note" style={{fontSize:'16px',color:'#9ca3af'}}>//* Your note goes here. *//</div>
    </div>
    <div className="tms-cell reveal" style={{textAlign:'center',flex:'1'}}>
      <div className="tms-val" data-count="1000" style={{fontSize:'40px',fontFamily:'JetBrains Mono',color:'#ff6b00'}}>//</div>
      <div className="tms-label" style={{fontSize:'20px',color:'#ffffff'}}>//* Your label goes here. *//</div>
      <div className="tms-note" style={{fontSize:'16px',color:'#9ca3af'}}>//* Your note goes here. *//</div>
    </div>
    <div className="tms-cell reveal" style={{textAlign:'center',flex:'1'}}>
      <div className="tms-val" data-count="2000" style={{fontSize:'40px',fontFamily:'JetBrains Mono',color:'#ff6b00'}}>//</div>
      <div className="tms-label" style={{fontSize:'20px',color:'#ffffff'}}>//* Your label goes here. *//</div>
      <div className="tms-note" style={{fontSize:'16px',color:'#9ca3af'}}>//* Your note goes here. *//</div>
    </div>
  </div>
...
<section className="falt" style={{background:'#0f172a',color:'#fff',padding:'60px 0',position:'relative'}}>
  <div className="container">
    <header className="falt-head anim d0" style={{textAlign:'center',marginBottom:'40px'}}>
      <h2 style={{fontFamily:'JetBrains Mono',fontSize:'clamp(32px, 5vw, 60px)',fontWeight:600,lineHeight:1.2}}>
        Create High-Quality AI Images & Videos with ByteDance Generative Models
      </h2>
    ...
    {[
      ...
      },
      {
        ...
        features:['Text-to-Video Generation','Audio-Visual Synchronization','Multilingual Lip-Sync','Cinematic Camera Control','10× Faster Inference'],
        image_url:'/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/09-5d90f4cffc.png',
        video_url:''
      }
    ].map((section, i) => (
      <div key={i} className={`falt-block ${i%2 !== 0 ? 'flip' : ''} reveal`} style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'60px'}}>
        <div style={{flex:'1',marginRight:'20px'}}>
          <h3 className="falt-num" style={{color:'#ff6b00',fontSize:'28px',marginBottom:'10px'}}>0{i + 1}</h3>
          <h3 style={{fontFamily:'JetBrains Mono',fontSize:'24px',fontWeight:600,lineHeight:1.3}}>{section.title}</h3>
          <p className="falt-copy" style={{fontFamily:'googleSansFlex, "googleSansFlex Fallback"',fontSize:'18px',lineHeight:1.6,whiteSpace:'pre-line'}}>{section.description}</p>
          <div className="falt-chips" style={{marginTop:'10px',display:'flex',flexWrap:'wrap'}}>{
            section.features.map((feature, j) => (
              <span key={j} className="falt-chip" style={{background:'#1a1a1a',color:'#fff',padding:'5px 10px',borderRadius:'8px',marginRight:'5px',marginBottom:'5px',fontSize:'14px'}}>
                {feature}
              </span>
            ))
          }</div>
...
<section className="fig">
  ...
  <div className="container" style={{padding:'40px',background:'#111827', color:'#fff'}}>
    <div className="fig-head reveal">
      <h2 style={{fontFamily:'JetBrains Mono',fontSize:'clamp(32px,5vw,60px)',fontWeight:700}} className="anim d0">Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
      <p style={{fontFamily:'googleSansFlex',marginTop:'16px',fontSize:'18px',lineHeight:'1.5'}} className="anim d1">Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    </div>
    <div className="fig-grid reveal stagger-parent" style={{display:'flex',flexWrap:'wrap',marginTop:'32px'}}>
      {[{"title":"Advanced Text–Image Alignment","description":"Accurately translates prompts into visuals with improved semantic understanding."},{"title":"High-Resolution Output","description":"Generate native images up to 1K–4K resolution with strong visual fidelity."},{"title":"Superior Typographic Rendering","description":"Optimized for posters, ads, and text-heavy visual designs."},{"title":"Multi-Image Composition with Identity Preservation","description":"Combines multiple inputs while accurately maintaining subject consistency."},{"title":"Strong Structural Fidelity","description":"Maintains composition, layout, and scene structure with high precision."},{"title":"Text-to-Video Generation","description":"Create videos directly from text prompts."}].map((feature, index) => (
        <div key={index} className="fig-card" style={{flex:'1 1 calc(33.333% - 32px)',margin:'16px',padding:'24px',background:'#0f172a',borderRadius:'12px'}}>
          <div className="fig-icon" style={{width:'16px',height:'16px',marginBottom:'12px',color:'#ff6b00'}}>🌟</div>
          <h4 className="anim" style={{fontFamily:'JetBrains Mono',fontSize:'20px',fontWeight:600,marginBottom:'12px'}}>{feature.title}</h4>
          <p className="anim" style={{fontFamily:'googleSansFlex',fontSize:'16px',lineHeight:'1.4'}}>{feature.description}</p>
        </div>
      ))}
    </div>
  </div>
...
<section className="gvw" style={{background:'#111827', padding:'40px 20px'}}>
  <div className="gvw-head" style={{position:'relative', overflow:'hidden', borderRadius:'12px', marginBottom:'30px'}}>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
    <div style={{position:'relative', zIndex:1, padding:'20px', background:'rgba(0,0,0,0.4)', borderRadius:'12px'}}>
      <span className="eyebrow" style={{color:'#ff6b00', fontWeight:'bold'}}>Gallery Video Wall</span>
      <h2 style={{color:'white', fontFamily:'JetBrains Mono', fontSize:'clamp(32px, 5vw, 60px)', margin:'10px 0'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
      <p style={{color:'white', fontFamily:'googleSansFlex, "googleSansFlex Fallback"', lineHeight:'1.6'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    </div>
  </div>
  <div className="gvw-grid" style={{display:'flex', gap:'20px', flexWrap:'wrap'}}>
    {[
      ...
      <div className={`gvw-card anim-scale d${index}`} key={index} style={{flex:'1 1 calc(25% - 20px)', position:'relative'}}>
        <video className="gvw-video" autoPlay muted loop playsInline style={{width:'100%', borderRadius:'12px', display:'block'}}>
          <source src={video.url} type="video/mp4" />
        </video>
        <div className="gvw-caption" style={{color:'white', textAlign:'center', marginTop:'10px'}}>{video.caption}</div>
      </div>
    ))}
  </div>
</section>

<section className="wtj" style={{background:'#111827',padding:'60px 0'}}>
  <div className="wtj-inner" style={{maxWidth:'1200px',margin:'0 auto',padding:'0 20px'}}>
    <div className="wtj-top" style={{textAlign:'center',marginBottom:'40px'}}>
      <img src="https://beta.techjockey.com/c/kaspersky-office-security/assets/img/tj_logo.svg" alt="Techjockey" className="wtj-logo" style={{width:'150px',margin:'0 auto'}}/>
      <div className="wtj-divider" style={{height:'2px',background:'#ff6b00',width:'50px',margin:'20px auto'}} />
      <p className="wtj-tagline" style={{color:'#f3f4f6',fontFamily:'googleSansFlex',fontSize:'18px'}}>India's #1 B2B Software Marketplace</p>
    </div>
    <div className="wtj-grid" style={{display:'flex',flexWrap:'wrap',justifyContent:'space-between',gap:'20px'}}>

      {[{title:"Free Expert Consultation",description:"Get matched with the right software based on your needs."},
        {title:"Verified Reviews",description:"1000+ genuine customer reviews to guide your choice."},
        {title:"Best Price Guarantee",description:"Assured competitive pricing without compromise."},
        {title:"Dedicated Support",description:"Receive post-sale onboarding assistance from our team."}
      ].map((point, index) => (
        <div key={index} className="wtj-card reveal" style={{flex:'1 1 calc(25% - 20px)',background:'#1a1a1a',padding:'20px',borderRadius:'8px'}}>
          <div className="wtj-icon" style={{width:'40px',height:'40px',marginBottom:'20px',background:'#ff6b00',borderRadius:'20px'}}></div>
          <h4 style={{fontFamily:'JetBrains Mono',fontSize:'20px',color:'#f3f4f6',marginBottom:'10px'}}>{point.title}</h4>
          <p style={{color:'#e5e7eb',fontFamily:'googleSansFlex',fontSize:'16px'}}>{point.description}</p>
        </div>
      ))}
...
<section className="pdc" style={{background:'#111827',padding:'80px 0',position:'relative'}}>
  <div className="container">
    <div className="pdc-head anim d0" style={{textAlign:'center',color:'#fff'}}>
      <h2 style={{fontFamily:'JetBrains Mono',fontSize:'clamp(32px, 5vw, 60px)',marginBottom:'20px'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
      <p className="eyebrow" style={{color:'#ff6b00'}}>Flexible Pricing for Innovators</p>
    </div>
    <div className="pdc-grid" style={{display:'flex',justifyContent:'center',gap:'20px',marginTop:'40px'}}>
      {[
        {
        ...
        <div key={index} className={`pdc-card ${plan.highlighted ? 'featured' : ''}`} style={{background:'#1a1a1a',padding:'40px 30px',borderRadius:'12px',width:'100%',maxWidth:'400px'}}>
          {plan.highlighted && <div className="pdc-badge" style={{color:'#fff',background:'#ff6b00',padding:'5px 10px',borderRadius:'8px',marginBottom:'15px',display:'inline-block'}}>{plan.discount}</div>}
          <h3 className="pdc-plan" style={{color:'#fff',marginBottom:'15px'}}>{plan.name}</h3>
          <div className="pdc-price" style={{display:'flex',alignItems:'baseline',marginBottom:'20px'}}>
            <span className="pdc-amount" style={{fontSize:'24px',fontWeight:'bold',color:'#ff6b00'}}>
              {plan.price || "Contact for Pricing"}
            </span>
            {plan.originalPrice && <span className="pdc-old" style={{textDecoration:'line-through',color:'#888',marginLeft:'10px'}}>{plan.originalPrice}</span>}
          </div>
          <span className="pdc-period" style={{display:'block',color:'#888',marginBottom:'20px'}}>{plan.period}</span>
          <ul className="pdc-list" style={{listStyle:'none',padding:0,color:'#fff'}}>
            {plan.includes.map((feature, i) => (
              <li key={i} style={{marginBottom:'10px'}}>
                <svg className="pdc-check" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#ff6b00" width="16px" height="16px" style={{marginRight:'10px',verticalAlign:'middle'}}><path d="M20.285 2.837l-11.056 11.057-4.293-4.293-1.414 1.414 5.707 5.707 12.471-12.471z"/></svg>
                {feature}
              </li>
            ))}
          </ul>
          <button className="pdc-btn" style={{marginTop:'20px',padding:'10px 20px',border:'none',background:'#ff6b00',color:'#fff',borderRadius:'8px',cursor:'pointer'}}>Generate with AI</button>
        </div>
      ))}
    </div>
  </div>
...
<section className="tcar" style={{background:'#0f172a', color:'#fff', padding: '60px 20px'}}>
  {React.useEffect(() => {
    const t = setInterval(() => setSlide((p) => (p + 1) % testimonials.length), 4000);
    return () => clearInterval(t);
  }, [])}
  <div className="tcar-head">
    <h2 className="eyebrow" style={{color:'#ff6b00'}}>Testimonials</h2>
    <h3 className="anim d0" style={{fontFamily:"'JetBrains Mono', monospace", fontSize: 'clamp(32px, 5vw, 60px)'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h3>
    <p style={{fontFamily:"'googleSansFlex'", maxWidth: '720px', margin:'20px auto'}}>
      Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.
    </p>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
  </div>
  <div className="tcar-layout">
    <div className="tcar-rail">
      {[{quote:"Seedream allows us to generate high-resolution creative visuals from simple prompts. It has dramatically reduced our design turnaround time.","author":"Vaishali Saxena","role":"Creative Director"},
          ...
          {quote:"The multimodal editing capabilities in Seedream make it easy to refine images with precision.","author":"Anurag Malhotra","role":"Art Director"}].map((item, index) => (
        <div key={index} className="tcar-mini" style={{padding:'10px', background:'#1a1a1a', borderRadius:'10px', margin:'10px'}}>
          <div className="tcar-mini-stars" style={{color: '#ff6b00'}}>★★★★★</div>
          <p style={{fontFamily:"'googleSansFlex'", overflow:'hidden', textOverflow:'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical'}}>"{item.quote}"</p>
        </div>
      ))}
    </div>
    <div className="tcar-stage anim-scale d1">
      <div className="tcar-track" style={{display: 'flex', transform: `translateX(-${slide * 100}%)`, transition: 'transform 0.5s ease-in-out'}}>
        {[{quote:"Seedream allows us to generate high-resolution creative visuals from simple prompts. It has dramatically reduced our design turnaround time.","author":"Vaishali Saxena","role":"Creative Director"},
          ...
          {quote:"Techjockey made it easy to evaluate Seedream (AI Image Generation) and Seedance (AI Video Generation). The free demo helped us clearly understand the capabilities before making a decision.","author":"Ashutosh Singh","role":"Marketing Manager"},
          {quote:"From understanding our needs to arranging a free demo of Seedream and Seedance, Techjockey simplified the entire buying journey. Quick, smooth, and hassle-free.","author":"Shrimmi Saxena","role":"Creative Lead"}].map((testimony, i) => (
          <div key={i} className="tcar-slide" style={{flex: '0 0 100%', padding: '20px'}}>
            <div className="tcar-quote" style={{backgroundColor:'#1a1a1a', padding: '20px', borderRadius: '10px'}}>
              <div className="tcar-stars" style={{color:'#ff6b00', marginBottom: '10px'}}>★★★★★</div>
              <p className="tcar-text" style={{fontFamily:"'googleSansFlex'", marginBottom: '20px'}}>"{testimony.quote}"</p>
              <div className="tcar-author" style={{display: 'flex', alignItems: 'center'}}>
                <div className="tcar-avatar" style={{background: i % 2 ? '#ff6b00' : '#1a1a1a', color:'#fff', borderRadius:'50%', width:'40px', height:'40px', display:'flex', alignItems:'center', justifyContent:'center', marginRight:'10px'}}>
                  {`${testimony.author.split(' ')[0][0]}${testimony.author.split(' ')[1][0]}`}
                </div>
                <div>
                  <div className="tcar-name" style={{fontFamily:"'JetBrains Mono'", fontSize:'1rem'}}>{testimony.author}</div>
                  <div className="tcar-role" style={{fontFamily:"'googleSansFlex'", color:'#ff6b00'}}>{testimony.role}</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div className="tcar-dots" style={{display: 'flex', justifyContent: 'center', marginTop:'20px'}}>
    {[0, 1, 2, 3, 4].map((_, i) => (
      <div key={i} className={`tcar-dot ${i === slide ? 'active' : ''}`} style={{width: '10px', height: '10px', borderRadius: '50%', background: i === slide ? '#ff6b00' : '#1a1a1a', margin: '0 5px'}}/>
    ))}
  </div>
</section>

<section className="ctavb" style={{position: 'relative', overflow: 'hidden', background: '#111827', color: 'white', fontFamily: 'googleSansFlex, "googleSansFlex Fallback"'}}>
  <div className="ctavb-bg" style={{position: 'absolute', inset: 0, zIndex: 1}}>
    ...
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0, opacity:0.3}} />
  </div>
  <div className="ctavb-overlay" style={{position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2}}></div>
  <div className="ctavb-inner" style={{position: 'relative', zIndex: 3, padding: '4rem 2rem', textAlign: 'center'}}>
    <h2 className={`anim d0`} style={{fontSize: 'clamp(32px, 5vw, 60px)', fontFamily: 'JetBrains Mono', color: '#ffffff', wordBreak: 'normal', overflowWrap: 'normal', hyphens: 'none'}}>
      Create High-Quality AI Images & Videos with ByteDance Generative Models
    </h2>
    <p className={`anim d1`} style={{marginTop: '1rem', marginBottom: '2rem', fontSize: '1.125rem', lineHeight: '1.5', color: '#e5e7eb'}}>
      Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.
    </p>
    <a href="#lead-form" className={`ctavb-btn anim d2`} style={{background: '#ff6b00', borderRadius: '8px', padding: '1rem 2rem', color: '#ffffff', fontWeight: 'bold', textDecoration: 'none', display: 'inline-block'}}>
      Generate with AI
    </a>
  </div>
  <div style={{display: 'flex', justifyContent: 'space-between', marginTop: '3rem', padding: '0 2rem', position: 'relative', zIndex: 3}}>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/09-5d90f4cffc.png" alt="Feature 1" style={{width:'45%',borderRadius:'12px',display:'block'}} />
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/09-5d90f4cffc.png" alt="Feature 2" style={{width:'45%',borderRadius:'12px',display:'block'}} />
  </div>
</section>
