...
@keyframes fadeUp { from { opacity:0; transform:translateY(28px); } to { opacity:1; transform:translateY(0); } }
    ...
    <div>
     ...
     <nav className="sticky top-0 z-50 bg-gray-900/90 backdrop-blur-xl border-b border-white/10 py-3.5">
        <div className="max-w-6xl mx-auto px-6 flex justify-between items-center gap-4">
          <span className="font-extrabold text-xl text-[#b3713f]">Seedream 4.5 and Seedance 1.5 Pro</span>
  ...
  <video src="/output/generated-assets/ds_1778136214158_38a0f50d/08-8c93b9a6e6.mp4" className="hvf-video" autoPlay loop muted playsInline />
  <div className="hvf-overlay" />
  <div className="hvf-content">
    <div className="hvf-inner">
      <h1 className="hvf-h1 anim d0">
        Create High-Quality AI Images & Videos with ByteDance Generative Models <span className="hvf-accent">Seedream 4.5 and Seedance 1.5 Pro</span>
      </h1>
      ...
      <div className="hvf-chips anim d2">
        <input type="text" placeholder="Enter your query" className="hvf-input" />
        <button className="hvf-btn">{cta_text}</button>
        <div className="hvf-proof">
          <div className="hvf-avatars">
            <div className="hvf-av" style={{ backgroundColor: '#f8b195' }}>JD</div>
            <div className="hvf-av" style={{ backgroundColor: '#f67280' }}>AB</div>
            <div className="hvf-av" style={{ backgroundColor: '#c06c84' }}>CD</div>
            <div className="hvf-av" style={{ backgroundColor: '#6c5b7b' }}>EF</div>
          </div>
          <div className="hvf-stars">
            {[...Array(5)].map((_, i) => (
              <svg key={i} xmlns="http://www.w3.org/2000/svg" fill="gold" viewBox="0 0 24 24" stroke="none" className="w-4 h-4">
                <path d="M12 18l-5.555 3.689L8 14.62 4 10h5.334L12 4l2.667 6H20l-4 4.62 1.556 7.069z" />
              </svg>
            ))}
          </div>
...
<section className="falt">
  <div className="container">
    <div className="falt-head anim d0">
      <span className="eyebrow">Innovative Solutions</span>
      <h2 className="text-4xl font-bold tracking-tight leading-tight text-[#111827]">Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    </div>
    
    {sections.map((section, index) => (
      <div key={index} className={`falt-block ${index % 2 !== 0 ? "flip" : ""} reveal`}>
        <span className="falt-num">{"0" + (index + 1)}</span>
        <h3 className="text-3xl font-semibold mb-3">{section.title}</h3>
        <p className="text-lg leading-relaxed text-[#4b5563] mb-5">{section.description}</p>
        <div className="falt-chips mb-5">
          {section.features.map((feature, idx) => (
            <span key={idx} className="falt-chip px-3 py-1.5 bg-[#b3713f]/10 border border-[#b3713f]/25 text-[#b3713f] text-xs font-bold tracking-wider uppercase mr-2">{feature}</span>
          ))}
        </div>
        <div className="falt-visual">
          {section.video_url ? (
            <video src={section.video_url} className="w-full rounded-xl shadow-md" controls></video>
          ) : (
            <img src={section.image_url} alt={section.title} className="w-full rounded-xl shadow-md" />
          )}
        </div>
      </div>
    ))}
...
<section className="fig">
  <div className="max-w-6xl mx-auto px-6">
    <div className="fig-head">
      <span className="inline-flex items-center gap-1.5 rounded-full bg-[#b3713f]/10 border border-[#b3713f]/25 text-[#b3713f] px-3 py-1 text-xs font-bold tracking-wider uppercase">section_label</span>
    ...
    <div className="fig-grid grid gap-6 mt-12 stagger-parent">
      {[{"title":"Advanced Text–Image Alignment","description":"Accurately translates prompts into visuals with improved semantic understanding."},{"title":"High-Resolution Output","description":"Generate native images up to 1K–4K resolution with strong visual fidelity."},{"title":"Superior Typographic Rendering","description":"Optimized for posters, ads, and text-heavy visual designs."},{"title":"Multi-Image Composition with Identity Preservation","description":"Combines multiple inputs while accurately maintaining subject consistency."},{"title":"Strong Structural Fidelity","description":"Maintains composition, layout, and scene structure with high precision."},{"title":"Key Capabilities of Seedance 1.5 Pro","description":"Seedance enables professional-grade AI video production with narrative coherence and realistic motion."}].map((feature, index) => (
        <div className="fig-card rounded-2xl border border-gray-200 bg-white shadow-sm p-6 animate-opacity translate-y-10" key={index}>
          <div className="fig-icon mb-4">
            {/* Example SVG - Replace with relevant SVG icon for each feature */}
            <svg width="16" height="16" fill="var(--accent)" className="text-[#b3713f]"><circle cx="8" cy="8" r="8" /></svg>
          </div>
          <h4 className="font-semibold text-gray-700">{feature.title}</h4>
          <p className="text-sm leading-relaxed text-gray-500 mt-2">{feature.description}</p>
        </div>
      ))}
    </div>
  </div>
  ...
  <div className="container">
    ...
    <div className="gvw-grid">
      {[
        ...
        <div className="gvw-card" key={index} data-anime="anim-scale" style={{ animationDelay: `${index * 100}ms` }}>
          <video className="gvw-video" src={video.url} autoPlay muted loop playsInline></video>
          <div className="gvw-caption">{video.caption}</div>
        </div>
      ))}
    </div>
  </div>
...
<section className="wtj py-24 bg-white">
  <div className="wtj-inner max-w-6xl mx-auto px-6">
    <div className="wtj-top flex flex-col items-center text-center mb-12">
      ...
      <p className="wtj-tagline text-lg font-semibold text-[#b3713f] mb-4">India's #1 B2B Software Marketplace</p>
      <div className="wtj-divider h-[1px] w-full bg-gray-200 mt-4"></div>
    </div>
    <div className="wtj-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 reveal">
      {[
        ...
        <div key={index} className="wtj-card anim d0 p-6 bg-white border border-gray-200 rounded-xl shadow-sm transition-transform hover:-translate-y-1.5 hover:shadow-md">
          <svg className="wtj-icon mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="48" height="48">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
          <h4 className="text-2xl font-bold mb-2">{point.title}</h4>
...
<section className="pdc">
  <div className="pdc-head reveal anim d0">
    <div className="eyebrow">section_label</div>
    <h2 className="h2">Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
  </div>
  <div className="pdc-grid grid gap-6 mt-12">
    {[
      {
      ...
      <div key={index} className={`pdc-card ${plan.highlighted ? "featured" : ""} anim d${index}`}>
        {plan.highlighted && (
          <div className="pdc-badge">{plan.discount || "Special Offer"}</div>
        )}
        ...
        <ul className="pdc-list">
          {plan.includes.map((include, i) => (
            <li key={i} className="flex items-center">
              <svg className="pdc-check" width="20" height="20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.5 14l-5-5L0 11.5l7.5 7.5L20 5 17.5 2.5l-10 10z" fill="#b3713f" />
              </svg>
              <span>{include}</span>
            </li>
          ))}
        </ul>
        <button className="pdc-btn Primary CTA">Learn More</button>
      </div>
    ))}
  </div>
</section>
...
<section className="tcar py-24 bg-gray-50">
  <div className="container anim d0">
    <span className="eyebrow">High-Quality Creation</span>
    <h2 className="h2 mt-4 tcar-head">Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    <p className="body-text mt-4 mb-12">Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    <div className="tcar-layout">
      <div className="tcar-rail">
        {testimonials.slice(0, 3).map((testimony, idx) => (
          <div className="tcar-mini" key={idx}>
            <p className="tcar-mini-stars">{'★★★★★'}</p>
            <p className="tcar-text">{`${testimony.quote.substring(0, 80)}...`}</p>
          </div>
        ))}
      </div>
        ...
        <div className="tcar-track" style={{ transform: `translateX(-${slide * 100}%)` }}>
          {testimonials.map((testimony, idx) => (
            <div className="tcar-slide" key={idx}>
              <p className="tcar-quote">{testimony.quote}</p>
              <div className="tcar-author">
                <div className="tcar-avatar" style={{ backgroundColor: idx % 2 === 0 ? '#dfe7fd' : '#fce7f3' }}>
                  {testimony.author.split(' ').map(name => name[0]).join('')}
                </div>
                <div>
                  <p className="tcar-name">{testimony.author}</p>
                  <p className="tcar-role">{`${testimony.role}${testimony.company ? `, ${testimony.company}` : ''}`}</p>
                </div>
              </div>
    ...
    <div className="tcar-dots mt-8">
      {testimonials.map((_, idx) => (
        <button key={idx} onClick={() => setSlide(idx)} className={`tcar-dot ${slide === idx ? 'active' : ''}`}></button>
      ))}
    </div>
  </div>
</section>
...
<section className="ctavb">
  <div className="ctavb-bg">
    <video autoPlay muted loop playsInline>
      <source src="/output/generated-assets/ds_1778136214158_38a0f50d/08-8c93b9a6e6.mp4" type="video/mp4" />
    </video>
  </div>
  <div className="ctavb-overlay"></div>
  <div className="ctavb-inner">
    <h2 className="text-4xl font-bold tracking-tight leading-tight anim d0">Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    <p className="text-lg leading-relaxed text-gray-500 anim d1">Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    <a href="#lead-form" className="ctavb-btn inline-flex items-center justify-center rounded-xl text-sm font-semibold h-11 px-6 bg-[#b3713f] text-white shadow hover:-translate-y-0.5 hover:opacity-90 transition-all focus-visible:outline-none anim d2">Generate with AI</a>
  </div>
</section>
