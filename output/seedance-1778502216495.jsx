  ...
  return (
    <div>
     ...
     <nav style={{position:'sticky',top:0,zIndex:50,background:'rgba(15,23,42,0.92)',backdropFilter:'blur(16px)',borderBottom:'1px solid rgba(255,255,255,0.1)',padding:'12px 0'}}>
      ...
      <section className="hvf" style={{display: 'flex', background: '#0f172a', padding: '20px'}}>
  <div className="hvf-content" style={{flex: '1 1 50%', position: 'relative', paddingRight: '20px'}}>
    <video autoPlay muted loop playsInline className="hvf-video" style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0, zIndex: -1}}>
      <source src="https://cdn.web.imagine.art/imagine-one/cdge-v2-prompt/Seedance/seedance-hero.mp4" type="video/mp4" />
    </video>
    <div className="hvf-overlay" style={{background: 'rgba(15, 23, 42, 0.8)', position: 'absolute', inset: 0}}></div>
    <div className="hvf-inner">
      <h1 className="hvf-h1 anim d0" style={{margin: '0 0 20px', fontFamily: 'Plus Jakarta Sans', fontSize: 'clamp(32px, 5vw, 60px)', lineHeight: 1.2, color: '#ffffff'}}>
        Create High-Quality AI Images & Videos with <span className="hvf-accent" style={{color: '#ff6b00'}}>Seedream 4.5 and Seedance 1.5 Pro by ByteDance</span> Generative Models
      </h1>
      <p className="hvf-desc anim d1" style={{marginBottom: '20px', fontFamily: 'googleSansFlex, "googleSansFlex Fallback"', fontSize: '16px', color: '#ffffff'}}>
      ...
      <div className="hvf-chips" style={{display: 'flex', marginBottom: '20px'}}>
        {["Seedream 4.5", "Seedance 1.5 Pro"].map((chip, index) => (
          <div key={index} className="hvf-chip" style={{background: '#111827', color: '#ffffff', padding: '5px 10px', marginRight: '10px', borderRadius: '20px', fontSize: '14px'}}>
            {chip}
          </div>
        ))}
      </div>
      <button className="hvf-btn" style={{background: '#ff6b00', color: '#ffffff', padding: '10px 20px', border: 'none', borderRadius: '5px', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans', fontSize: '16px'}}>
        Generate with AI
      </button>
    </div>
  </div>
  <div style={{flex: '1 1 50%', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
    <form className="anim d2" style={{background: '#111827', padding: '30px', borderRadius: '12px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)'}}>
      <div style={{marginBottom: '20px'}}>
        <input type="text" placeholder="Name" className="hvf-input" style={{width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #333'}} />
      </div>
      <div style={{marginBottom: '20px'}}>
        <input type="email" placeholder="Email" className="hvf-input" style={{width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #333'}} />
      </div>
      <div style={{marginBottom: '20px'}}>
        <input type="tel" placeholder="Phone" className="hvf-input" style={{width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #333'}} />
      </div>
      <div style={{marginBottom: '30px'}}>
        <input type="text" placeholder="Company" className="hvf-input" style={{width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #333'}} />
      </div>
      <button type="submit" className="hvf-btn" style={{background: '#ff6b00', color: '#ffffff', padding: '10px', width: '100%', border: 'none', borderRadius: '5px', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans', fontSize: '16px'}}>
        Generate with AI
      </button>
    </form>
  </div>
...
<section className="tms" style={{backgroundColor:'#111827', padding:'2rem', position:'relative', overflow:'hidden'}}>
  <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0,zIndex:0}}/>
  <div className="tms-grid" style={{display: 'flex', gap: '1rem', position:'relative', zIndex:1}}>
    <div className="tms-cell reveal" style={{flex:'1', background:'#0f172a',padding:'1rem',borderRadius:'8px',display:'flex',flexDirection:'column',alignItems:'center', justifyContent:'center'}}>
      <div className="tms-val" data-count="100+" style={{fontSize:'2rem',color:'#ff6b00',fontWeight:'bold'}}>100+</div>
      <div className="tms-label" style={{fontSize:'1rem',color:'#ffffff',marginTop:'0.5rem'}}>High-Resolution Outputs</div>
      <div className="tms-note" style={{fontSize:'0.875rem',color:'#9ca3af',marginTop:'0.25rem'}}>//</div>
    </div>
    <div className="tms-cell reveal" style={{flex:'1', background:'#0f172a',padding:'1rem',borderRadius:'8px',display:'flex',flexDirection:'column',alignItems:'center', justifyContent:'center'}}>
      <div className="tms-val" data-count="150+" style={{fontSize:'2rem',color:'#ff6b00',fontWeight:'bold'}}>150+</div>
      <div className="tms-label" style={{fontSize:'1rem',color:'#ffffff',marginTop:'0.5rem'}}>Clients Served Globally</div>
      <div className="tms-note" style={{fontSize:'0.875rem',color:'#9ca3af',marginTop:'0.25rem'}}>//</div>
    </div>
    <div className="tms-cell reveal" style={{flex:'1', background:'#0f172a',padding:'1rem',borderRadius:'8px',display:'flex',flexDirection:'column',alignItems:'center', justifyContent:'center'}}>
      <div className="tms-val" data-count="300K" style={{fontSize:'2rem',color:'#ff6b00',fontWeight:'bold'}}>300K</div>
      <div className="tms-label" style={{fontSize:'1rem',color:'#ffffff',marginTop:'0.5rem'}}>Images Generated</div>
      <div className="tms-note" style={{fontSize:'0.875rem',color:'#9ca3af',marginTop:'0.25rem'}}>//</div>
    </div>
  </div>
</section>

<section className="falt" style={{background:'#111827',padding:'60px 0',position:'relative',overflow:'hidden'}}>
  <div className="falt-head anim d0" style={{textAlign:'center',marginBottom:'40px'}}>
    <h2 style={{color:'#ff6b00',fontFamily:'Plus Jakarta Sans',fontSize:'clamp(32px, 5vw, 60px)',fontWeight:'600',marginBottom:'16px'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
  </div>
  {[
    {
      num: "01",
      ...
      image_url: "/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/09-5d90f4cffc.png",
      video_url: ""
    },
    {
      num: "02",
      ...
      image_url: "/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/12-5d90f4cffc.png",
      video_url: ""
    }
  ].map((section, index) => (
      <div key={index} className={`falt-block reveal ${index % 2 === 1 ? 'flip' : ''}`} style={{marginBottom:'80px',display:'flex',alignItems:'center',flexDirection:index % 2 === 1 ? 'row-reverse' : 'row'}}>
        <div style={{flex:'1',padding:'20px'}}>
          <div className="falt-num" style={{color:'#ff6b00',fontFamily:'Plus Jakarta Sans',fontSize:'22px',fontWeight:'500',marginBottom:'8px'}}>0{index + 1}</div>
          <h3 style={{color:'#fff',fontFamily:'Plus Jakarta Sans',fontSize:'26px',fontWeight:'600',marginBottom:'16px'}}>{section.title}</h3>
          <p style={{color:'#9CA3AF',fontFamily:'googleSansFlex',fontSize:'16px',marginBottom:'20px'}}>{section.description}</p>
          <div className="falt-chips" style={{display:'flex',flexWrap:'wrap',gap:'10px'}}>
            {section.features.map((feature, i) => (
              <span key={i} className="falt-chip" style={{background:'#1F2937',color:'#ff6b00',padding:'6px 12px',borderRadius:'8px',fontSize:'14px'}}>{feature}</span>
            ))}
          </div>
        </div>
...
<section className="mns" style={{backgroundColor:'#111827',padding:'40px 20px',position:'relative'}}>
  <div className="mns-head" style={{textAlign:'center',marginBottom:'20px'}}>
    <span className="mns-label" style={{color:'#ff6b00',fontSize:'20px',letterSpacing:'1px'}}>FEATURES</span>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
  </div>
  <div className="mns-grid" style={{display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:'10px'}}>
    {[
      {publication: "TechCrunch", headline: "ByteDance's Seedream 4.5 Revolutionizes Image Generation"},
      {publication: "The Verge", headline: "Seedance 1.5 Pro by ByteDance: Pioneering AI Video Creation"},
      {publication: "Forbes", headline: "Unlocking New Heights in AI with ByteDance's Latest Models"}
    ].map((item, i) => (
      <div className="mns-card" key={i} style={{background:'#0f172a',padding:'20px',borderRadius:'12px',width:'calc(33% - 20px)',minWidth:'280px',flex:'1 0 calc(33% - 20px)',animationDelay:`${i * 0.1}s`}}>
        <div className="mns-pub" style={{color:'#ff6b00',fontWeight:'bold',marginBottom:'10px'}}>{item.publication}</div>
        <div className="mns-text" style={{color:'#ffffff',fontSize:'16px'}}>"{item.headline}"</div>
      </div>
    ))}
  </div>
</section>
...
<section className="fig">
  <div className="fig-head" style={{position: 'relative', overflow: 'hidden', background: '#0f172a', padding: '4rem 0'}}>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width: '100%', height: '100%', objectFit: 'cover', display: 'block', position: 'absolute', inset: 0}}/>
    <div style={{position: 'relative', zIndex: 1, textAlign: 'center', color: '#fff', maxWidth: '800px', margin: '0 auto'}}>
        <div className="eyebrow" style={{marginBottom: '1rem', color: '#ff6b00'}}>Features</div>
        <h2 style={{fontSize: 'clamp(32px, 5vw, 60px)', fontFamily: 'Plus Jakarta Sans', marginBottom: '1rem'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
        <p style={{fontFamily: 'googleSansFlex, "googleSansFlex Fallback"', color: '#e5e7eb', marginBottom: '3rem'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    </div>
  </div>
  <div className="fig-grid stagger-parent" style={{display: 'flex', flexWrap: 'wrap', gap: '1.5rem', padding: '4rem', background: '#111827'}}>
    {[{"title":"Advanced Capabilities of Seedream 4.5 by ByteDance","description":"From accurate text rendering to consistent image editing and multi-image composition, Seedream 4.5 powers high-quality, "},{"title":"Advanced Text–Image Alignment","description":"Accurately translates prompts into visuals with improved semantic understanding."},{"title":"High-Resolution Output","description":"Generate native images up to 1K–4K resolution with strong visual fidelity."},{"title":"Superior Typographic Rendering","description":"Optimized for posters, ads, and text-heavy visual designs."},{"title":"Multi-Image Composition with Identity Preservation","description":"Combines multiple inputs while accurately maintaining subject consistency."},{"title":"Strong Structural Fidelity","description":"Maintains composition, layout, and scene structure with high precision."}].map((feature, index) => (
      <div key={index} className="fig-card" style={{background: '#0f172a', borderRadius: '12px', padding: '2rem', flex: '1 1 calc(33% - 1.5rem)', minWidth: '280px', transition: 'opacity 0.3s ease, transform 0.3s ease', opacity: 0, transform: 'translateY(50px)'}}>
        <div className="fig-icon" style={{marginBottom: '1rem'}}>
          <svg width="16" height="16" fill="var(--accent)" xmlns="http://www.w3.org/2000/svg"><circle cx="8" cy="8" r="8"/></svg>
        </div>
        <h4 style={{fontFamily: 'Plus Jakarta Sans', color: '#ff6b00', marginBottom: '0.5rem'}}>{feature.title}</h4>
        <p style={{fontFamily: 'googleSansFlex, "googleSansFlex Fallback"', color: '#e5e7eb'}}>{feature.description}</p>
      </div>
    ))}
  </div>
</section>

<section className="gvw" style={{background:'#0f172a', color:'#fff', padding:'60px 0'}}>
  <div className="container" style={{position:'relative', overflow:'hidden', borderRadius:'12px', marginBottom:'40px'}}>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%', height:'100%', objectFit:'cover', display:'block', position:'absolute', inset:0}}/>
  </div>
  <div className="text-center">
    <div className="eyebrow" style={{textTransform:'uppercase', marginBottom:'10px'}}>Gallery Video Wall</div>
    <h2 className="gvw-head" style={{fontSize:'clamp(32px, 5vw, 60px)', fontFamily:'Plus Jakarta Sans', marginBottom:'20px'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    <p style={{fontFamily:'googleSansFlex, "googleSansFlex Fallback"', fontSize:'16px', maxWidth:'800px', margin:'0 auto 40px'}}>
      Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.
    </p>
    <div className="gvw-grid" style={{display:'flex', flexWrap:'wrap', justifyContent:'center', gap:'20px'}}>
      {[{"url":"https://cdn.web.imagine.art/imagine-one/cdge-v2-prompt/Seedance/seedance-hero.mp4","caption":"Seedream 4.5"},{"url":"https://cdn.web.imagine.art/imagine-one/CDGE-V3/seedance/MultiModalInput_1.mp4","caption":"Seedance 1.5 Pro"},{"url":"https://cdn.web.imagine.art/imagine-one/CDGE-V3/seedance/ConsistentCharacters.mp4","caption":"Demo 3"},{"url":"https://cdn.web.imagine.art/imagine-one/CDGE-V3/seedance/ReferenceControl.mp4","caption":"Demo 4"}].map((video, index) => (
        <div key={index} className="gvw-card anim-scale" style={{flex:'1 0 220px', maxWidth:'220px'}}>
          <video className="gvw-video" autoPlay muted loop playsInline style={{width:'100%', borderRadius:'12px', overflow:'hidden'}}>
            <source src={video.url} type="video/mp4"/>
          </video>
          <div className="gvw-caption" style={{textAlign:'center', marginTop:'10px'}}>{video.caption}</div>
        </div>
      ))}
    </div>
  </div>
...
<section className="wtj" style={{background:'#0f172a',color:'#fff',padding:'60px 20px',position:'relative'}}>
  <div className="wtj-inner" style={{maxWidth:'1200px',margin:'0 auto',position:'relative',zIndex:1}}>
    <div className="wtj-top" style={{textAlign:'center',marginBottom:'40px'}}>
      <img src="https://beta.techjockey.com/c/kaspersky-office-security/assets/img/tj_logo.svg" alt="Techjockey" className="wtj-logo" style={{marginBottom:'20px'}}/>
      <h3 className="wtj-tagline" style={{fontFamily:'Plus Jakarta Sans',fontSize:'24px',fontWeight:600}}>India's #1 B2B Software Marketplace</h3>
    </div>
    <div className="wtj-divider" style={{margin:'40px 0',height:'2px',background:'#333',opacity:0.6}}></div>
    <div className="wtj-grid" style={{display:'flex',flexWrap:'wrap',justifyContent:'space-between',gap:'20px'}} className="stagger-parent">
      {[
        ...
        <div className="wtj-card" style={{flex:'0 0 calc(50% - 10px)',padding:'20px',background:'#111827',borderRadius:'12px'}} key={index}>
          <div className="wtj-icon" style={{fontSize:'30px',color:'#ff6b00',marginBottom:'10px'}}>★</div>
          <h4 style={{fontFamily:'googleSansFlex',fontSize:'20px',fontWeight:500,marginBottom:'10px'}}>{point.title}</h4>
          <p style={{fontFamily:'googleSansFlex',fontSize:'16px',lineHeight:'1.5',color:'#ccc'}}>{point.description}</p>
        </div>
      ))}
    </div>
  </div>
...
<section className="pdc" style={{background:'#111827', padding:'60px 0', position:'relative'}}>
  <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0,zIndex:0}}/>
  <div className="container" style={{position:'relative',zIndex:1}}>
    <div className="pdc-head anim d0" style={{textAlign:'center', marginBottom:'40px'}}>
      <span className="eyebrow" style={{display:'block', color:'#ff6b00', fontSize:'18px', marginBottom:'10px'}}>Our Plans</span>
      <h2 style={{fontSize:'clamp(32px,5vw,60px)', lineHeight:'1.2', color:'#ffffff', fontFamily:'Plus Jakarta Sans, sans-serif'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    </div>
    <div className="pdc-grid" style={{display:'flex', flexWrap:'wrap', gap:'20px'}}>
      {[{"name":"Seedream 4.5 (AI Image Generation)","price":"","originalPrice":"","discount":"","period":"per user/month","description":"","includes":["High-resolution image generation (up to 4K quality)","Text-to-image & multimodal image editing","Multi-image composition for complex visuals","Enhanced typographic rendering for posters, ads & text-heavy designs"],"highlighted":true},{"name":"Seedance 1.5 Pro (AI Video Generation)","price":"Starting at $1,000/month/","originalPrice":"","discount":"","period":"per user/month","description":"","includes":["Text-to-video generation with cinematic output","Native audio + video generation (synchronized)","Multilingual lip-sync capabilities","Fast inference for quicker video production"],"highlighted":false}].map((plan, index) => (
        <div key={index} className={`pdc-card ${plan.highlighted ? 'featured' : ''}`} style={{flex:'1 1 calc(50% - 20px)', background:'#0f172a', borderRadius:'8px', padding:'20px', boxShadow:'0 4px 10px rgba(0,0,0,0.1)'}}>
          {plan.highlighted && <div className="pdc-badge" style={{background:'#ff6b00', color:'#ffffff', fontSize:'14px', padding:'5px 10px', borderRadius:'4px', marginBottom:'10px'}}>Featured</div>}
          <h3 className="pdc-plan" style={{color:'#ffffff', fontFamily:'Plus Jakarta Sans, sans-serif', fontSize:'24px', marginBottom:'20px'}}>{plan.name}</h3>
          <div className="pdc-price" style={{marginBottom:'15px'}}>
            <span className="pdc-amount" style={{color:'#ffffff', fontSize:'20px', fontWeight:'bold'}}>{plan.price || 'Contact for Pricing'}</span>
            {plan.originalPrice && <span className="pdc-old" style={{color:'#8999b0', textDecoration:'line-through', marginLeft:'10px'}}>{plan.originalPrice}</span>}
          </div>
          <div className="pdc-period" style={{color:'#8999b0', marginBottom:'20px'}}>{plan.period}</div>
          <p className="pdc-desc" style={{color:'#8999b0', marginBottom:'20px'}}>{plan.description}</p>
          <ul className="pdc-list" style={{listStyleType:'none', paddingLeft:'0', color:'#ffffff'}}>
            {plan.includes.map((include, i) => (
              <li key={i} style={{display:'flex', alignItems:'flex-start', marginBottom:'10px'}}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="pdc-check" style={{width:'24px', height:'24px', marginRight:'10px', color:'#ff6b00'}}><polyline points="20 6 9 17 4 12"/></svg>
                <span>{include}</span>
              </li>
            ))}
          </ul>
          <button className="pdc-btn" style={{marginTop:'20px', padding:'10px 20px', background:'#ff6b00', border:'none', borderRadius:'4px', color:'#ffffff', cursor:'pointer', fontFamily:'Plus Jakarta Sans, sans-serif', fontSize:'16px'}}>Generate with AI</button>
        </div>
      ))}
    </div>
  </div>
...
<section className="tcar" style={{background:'#111827',color:'#fff',padding:'60px 20px',position:'relative'}}>
  {(() => {
    const [slide, setSlide] = React.useState(0);
    const testimonials = [
      {"quote":"Seedream allows us to generate high-resolution creative visuals from simple prompts. It has dramatically reduced our design turnaround time.","author":"Vaishali Saxena","role":"Creative Director","company":"","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/17-761b0d7272.webp"},
      {"quote":"Seedance’s ability to generate synchronized audio and video is incredibly powerful for storytelling and branded content.","author":"Vihaan Pandey","role":"Video Producer","company":"","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/04-6d75b26b02.webp"},
      {"quote":"The multimodal editing capabilities in Seedream make it easy to refine images with precision.","author":"Anurag Malhotra","role":"Art Director","company":"","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/22-67afb8c98a.webp"},
      {"quote":"Techjockey made it easy to evaluate Seedream (AI Image Generation) and Seedance (AI Video Generation). The free demo helped us clearly understand the capabilities before making a decision.","author":"Ashutosh Singh","role":"Marketing Manager","company":"","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/23-0518458b81.webp"},
    ...
    return (
      <React.Fragment>
        <div className="tcar-head anim d0" style={{marginBottom:'40px',textAlign:'center'}}>
          <h2 className="eyebrow" style={{color:'#ff6b00',marginBottom:'8px'}}>Testimonials</h2>
          <h3 style={{fontFamily:'Plus Jakarta Sans',fontSize:'clamp(32px,5vw,60px)',margin:'0',wordBreak:'normal',overflowWrap:'normal',hyphens:'none'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h3>
          <p style={{fontFamily:'googleSansFlex, "googleSansFlex Fallback"',fontSize:'1.125rem',marginTop:'16px',color:'#d1d5db',maxWidth:'700px',marginLeft:'auto',marginRight:'auto'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
        </div>
        
        <div className="tcar-layout" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'40px'}}>
          <div className="tcar-rail" style={{display:'flex',overflow:'hidden',maxWidth:'700px'}}>
            {testimonials.slice(0, 3).map((t, i) => (
              <div key={i} className="tcar-mini" style={{flex:'1 0 auto',minWidth:'220px',padding:'20px',background:'#1f2937',borderRadius:'12px',marginRight:'12px'}}>
                <div className="tcar-mini-stars" style={{color:'#ff6b00',marginBottom:'8px'}}>★★★★★</div>
                <p style={{margin:'0 0 8px',color:'#e5e7eb',overflow:'hidden',textOverflow:'ellipsis',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{t.quote}</p>
                <div className="tcar-avatar" style={{display:'flex',alignItems:'center'}}>
                  <img src={t.avatar || ''} alt={t.author} style={{borderRadius:'50%',width:'36px',height:'36px',marginRight:'8px'}}/>
                  <div style={{display:'flex',flexDirection:'column'}}>
                    <span className="tcar-name" style={{color:'#fff',fontWeight:'bold'}}>{t.author}</span>
                    <span className="tcar-role" style={{color:'#9ca3af'}}>{t.role}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="tcar-stage anim-scale d1" style={{maxWidth:'700px',position:'relative'}}>
            <div className="tcar-track" style={{display:'flex',transition:'transform 0.5s ease',transform:`translateX(-${slide * 100}%)`}}>
              {testimonials.map((t, i) => (
                <div key={i} className="tcar-slide" style={{flex:'0 0 100%',padding:'20px',textAlign:'center',background:'#1f2937',borderRadius:'12px'}}>
                  <p className="tcar-quote" style={{color:'#e5e7eb',fontSize:'1.125rem',marginBottom:'20px'}}>"{t.quote}"</p>
                  <div className="tcar-avatar" style={{display:'flex',alignItems:'center',justifyContent:'center',marginBottom:'12px'}}>
                    <img src={t.avatar || ''} alt={t.author} style={{borderRadius:'50%',width:'56px',height:'56px',marginRight:'12px'}}/>
                    <div style={{textAlign:'left'}}>
                      <span className="tcar-name" style={{color:'#fff',fontWeight:'bold'}}>{t.author}</span>
                      <span className="tcar-role" style={{color:'#9ca3af',display:'block'}}>{t.role}</span>
                    </div>
                  </div>
                  <div className="tcar-stars" style={{color:'#ff6b00'}}>★★★★★</div>
                </div>
              ))}
            </div>
          </div>

          <div className="tcar-dots" style={{display:'flex',justifyContent:'center',marginTop:'12px'}}>
            {testimonials.map((_, i) => (
              <div key={i} className={`tcar-dot ${i === slide ? 'active' : ''}`} style={{width:'10px',height:'10px',borderRadius:'50%',background:'#e5e7eb',marginRight:'8px',cursor:'pointer',opacity:i === slide ? 1 : 0.5}} onClick={() => setSlide(i)}></div>
            ))}
          </div>
        </div>
      </React.Fragment>
...
<section className="ctavb" style={{backgroundColor:'#0f172a',position:'relative',overflow:'hidden'}}>
  <div className="ctavb-bg" style={{position:'absolute',inset:0,zIndex:-1}}>
    <video autoPlay muted loop playsInline style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}>
      <source src="https://cdn.web.imagine.art/imagine-one/cdge-v2-prompt/Seedance/seedance-hero.mp4" type="video/mp4"/>
    </video>
    <div className="ctavb-overlay" style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.5)'}}></div>
  </div>
  <div className="ctavb-inner" style={{position:'relative',zIndex:1,padding:'100px 20px',textAlign:'center'}}>
    <h2 className="anim d0" style={{color:'#fff',fontFamily:'Plus Jakarta Sans',fontSize:'clamp(32px,5vw,60px)',fontWeight:700,marginBottom:'20px'}}>
      Create High-Quality AI Images & Videos with ByteDance Generative Models
    </h2>
    <p className="anim d1" style={{color:'#ccc',fontFamily:'googleSansFlex',maxWidth:'600px',margin:'0 auto 40px'}}>
      Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.
    </p>
    <a href="#lead-form" className="ctavb-btn anim d2" style={{display:'inline-block',background:'#ff6b00',color:'#fff',padding:'15px 30px',borderRadius:'8px',fontFamily:'googleSansFlex',fontWeight:700,textDecoration:'none'}}>
      Generate with AI
    </a>
  </div>
  <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro-by-bytedance/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro by ByteDance" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
