...
<section className="tms" style={{backgroundColor: '#111827', padding: '40px 0'}}>
  <div className="container">
    <div style={{position:'relative',width:'100%',paddingBottom:'56.25%',marginBottom:'40px'}}>
      <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
    </div>
    <div className="tms-grid" style={{display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: '20px'}}>
      <div className="tms-cell stagger-parent" style={{flex: '1 1 30%', minWidth: '220px', backgroundColor: '#1a1a1a', borderRadius: '12px', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
        <div className="tms-val" data-count="123" style={{fontSize: '40px', fontWeight: 'bold', color: '#ff6b00'}}>123</div>
        <div className="tms-label" style={{fontSize: '18px', color: '#FFFFFF', margin: '10px 0'}}>High Resolutions</div>
        <div className="tms-note" style={{fontSize: '14px', color: '#d1d5db'}}>Ability to generate // detailed visuals.</div>
      </div>
      <div className="tms-cell stagger-parent" style={{flex: '1 1 30%', minWidth: '220px', backgroundColor: '#1a1a1a', borderRadius: '12px', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
        <div className="tms-val" data-count="456" style={{fontSize: '40px', fontWeight: 'bold', color: '#ff6b00'}}>456</div>
        <div className="tms-label" style={{fontSize: '18px', color: '#FFFFFF', margin: '10px 0'}}>Models Trained</div>
        <div className="tms-note" style={{fontSize: '14px', color: '#d1d5db'}}>Over // unique models trained.</div>
      </div>
      <div className="tms-cell stagger-parent" style={{flex: '1 1 30%', minWidth: '220px', backgroundColor: '#1a1a1a', borderRadius: '12px', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
        <div className="tms-val" data-count="789" style={{fontSize: '40px', fontWeight: 'bold', color: '#ff6b00'}}>789</div>
        <div className="tms-label" style={{fontSize: '18px', color: '#FFFFFF', margin: '10px 0'}}>Satisfied Users</div>
        <div className="tms-note" style={{fontSize: '14px', color: '#d1d5db'}}>Join over // satisfied users.</div>
      </div>
    </div>
  </div>
</section>

<section className="falt" style={{backgroundColor:'#111827',color:'#ffffff',padding:'80px 0'}}>
  <div className="container">
    <div className="falt-head anim d0" style={{color:'#ff6b00',fontFamily:'Plus Jakarta Sans',marginBottom:'40px'}}>
      <h2 style={{fontSize:'clamp(32px,5vw,60px)',wordBreak:'normal',overflowWrap:'normal',hyphens:'none'}}>
        Create High-Quality AI Images & Videos with ByteDance Generative Models
      </h2>
    </div>
    {[{"num":"01","title":"AI Image Generation with Seedream 4.5","description":"Seedream 4.5 is a high-performance multimodal image generation system designed to produce high-resolution, high-fidelity images from text prompts and visual inputs.\nThe model unifies text-to-image synthesis, image editing, and multi-image composition within a single framework.","features":["Advanced Text–Image Alignment","High-Resolution Output","Superior Typographic Rendering","Multi-Image Composition with Identity Preservation","Strong Structural Fidelity"],"image_url":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/12-f1484ed1a9.png","video_url":""},{"num":"02","title":"AI Video Generation with Seedance 1.5 Pro by Bytedance","description":"Seedance 1.5 Pro is a next-generation generative model designed for native audio-visual generation, enabling synchronized creation of video and sound together.\nBuilt on a dual-branch diffusion transformer architecture, the model integrates cross-modal learning to produce coherent visual and audio outputs.","features":["Key Capabilities of Seedance 1.5 Pro"],"image_url":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/09-5d90f4cffc.png","video_url":""}].map((section, index) => (
      <div key={index} className={`falt-block ${index % 2 !== 0 ? 'flip' : ''} reveal`} style={{marginBottom:'40px',display:'flex',gap:'20px',alignItems:'center'}}>
        <div style={{flex:'1'}}>
          <p className="falt-num" style={{fontSize:'24px',color:'#ff6b00',marginBottom:'16px'}}>{"0" + (index + 1)}</p>
          <h3 style={{fontFamily:'Plus Jakarta Sans',fontSize:'28px',marginBottom:'16px'}}>{section.title}</h3>
          <p className="falt-copy" style={{fontFamily:'Inter',fontSize:'18px',lineHeight:'1.6',marginBottom:'24px'}}>{section.description}</p>
          <div className="falt-chips" style={{display:'flex',flexWrap:'wrap',gap:'10px'}}>
            {section.features.map((feature, featureIndex) => (
              <span key={featureIndex} className="falt-chip" style={{backgroundColor:'#ff6b00',borderRadius:'12px',padding:'8px 12px',color:'#ffffff'}}>
                {feature}
              </span>
            ))}
          </div>
        ...
        <div className="falt-visual" style={{flex:'1'}}>
          {index === 0 ? (
            <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/12-f1484ed1a9.png" alt={`Feature ${index + 1}`} style={{width:'100%',borderRadius:'12px',display:'block'}}/>
          ) : (
            <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/09-5d90f4cffc.png" alt={`Feature ${index + 1}`} style={{width:'100%',borderRadius:'12px',display:'block'}}/>
          )}
        </div>
      </div>
    ))}
    <div style={{position:'relative',width:'100%',height:'400px',marginTop:'40px',borderRadius:'12px',overflow:'hidden'}}>
      <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
    </div>
  </div>
</section>

<section className="fig" style={{background:'#111827',padding:'50px 0'}}>
  <div className="container">
    <div className="fig-head" style={{marginBottom:'40px',textAlign:'center'}}>
      <p className="eyebrow" style={{color:'#ff6b00',fontWeight:'bold'}}>Features</p>
      <h2 style={{color:'#fff',fontFamily:'Plus Jakarta Sans',fontSize:'2rem',marginBottom:'20px'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
      <p style={{color:'#cbd5e1',fontFamily:'Inter',maxWidth:'600px',margin:'0 auto'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    </div>
    <div className="fig-grid stagger-parent" style={{display:'grid',gap:'24px',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))'}}>
      {[
        ...
        <div key={i} className="fig-card" style={{background:'#1a1a1a',padding:'20px',borderRadius:'12px',boxShadow:'0 4px 8px rgba(0,0,0,0.1)'}}>
          <div className="fig-icon" style={{width:'16px',height:'16px',background:'#ff6b00',borderRadius:'50%',marginBottom:'12px'}}></div>
          <h4 style={{color:'#ff6b00',fontFamily:'Plus Jakarta Sans',fontSize:'1.25rem',marginBottom:'10px'}}>{feature.title}</h4>
          <p style={{color:'#cbd5e1',fontFamily:'Inter'}}>{feature.description}</p>
        </div>
      ))}
    </div>
    <div style={{position:'relative',maxWidth:'800px',margin:'40px auto',overflow:'hidden',borderRadius:'12px'}}>
      <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
    </div>
  </div>
</section>

<section className="gvw" style={{ backgroundColor: '#111827', padding: '50px 0' }}>
  <div className="gvw-head" style={{ textAlign: 'center', marginBottom: '40px' }}>
    <div className="eyebrow" style={{ color: '#ff6b00', fontSize: '18px', fontWeight: 'bold', marginBottom: '10px' }}>Our Gallery</div>
    <h2 style={{ fontSize: 'clamp(32px, 5vw, 60px)', color: '#fff', margin: '0 0 20px', fontFamily: 'Plus Jakarta Sans', wordBreak: 'normal', overflowWrap: 'normal', hyphens: 'none' }}>
      Create High-Quality AI Images & Videos with ByteDance Generative Models
    </h2>
    <p style={{ color: '#d1d5db', fontSize: '18px', maxWidth: '800px', margin: '0 auto' }}>
      Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.
    </p>
  </div>
  
  <div className="gvw-grid" style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '20px', maxWidth: '1200px', margin: '0 auto' }}>
    {[{url:"https://cdn.web.imagine.art/imagine-one/cdge/sora1.mp4",caption:"Seedream 4.5"},{url:"https://cdn.web.imagine.art/imagine-one/cdge/sora2.mp4",caption:"Seedance 1.5 Pro"},{url:"https://cdn.web.imagine.art/imagine-one/cdge/sora33.mp4",caption:"Demo 3"},{url:"https://cdn.web.imagine.art/remote-config/assets/video_effects/sd/podcast.mp4",caption:"Demo 4"}].map((video, i) => (
      <div key={i} className="gvw-card anim-scale" style={{ position: 'relative', width: '280px', height: '160px', marginBottom: '20px', overflow: 'hidden', borderRadius: '12px', flex: '1 1 calc(25% - 20px)' }}>
        <video className="gvw-video" autoPlay muted loop playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }}>
          <source src={video.url} type="video/mp4" />
        </video>
        <div className="gvw-caption" style={{ position: 'absolute', bottom: '10px', left: '10px', right: '10px', padding: '5px 10px', backgroundColor: 'rgba(0, 0, 0, 0.7)', color: '#ff6b00', borderRadius: '6px', fontSize: '14px', textAlign: 'center' }}>{video.caption}</div>
      </div>
    ))}
  </div>
</section>

<section className="wtj" style={{backgroundColor:'#111827',padding:'60px 20px'}}>
  <div className="wtj-inner" style={{maxWidth:'1200px',margin:'0 auto',position:'relative'}}>
    <div className="wtj-top" style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'40px'}}>
      <img src="https://beta.techjockey.com/c/kaspersky-office-security/assets/img/tj_logo.svg" alt="Techjockey" className="wtj-logo" style={{height:'50px'}} />
      <div style={{flexGrow:1, height:'1px', background:'#fff',opacity:'0.2',margin:'0 20px'}} className="wtj-divider"></div>
      <span className="wtj-tagline" style={{color:'#ff6b00',fontSize:'18px',fontWeight:'bold'}}>India's #1 B2B Software Marketplace</span>
    </div>
    <div className="wtj-grid stagger-parent" style={{display:'flex',gap:'20px',flexWrap:'wrap'}}>
      {[
        ...
        <div key={index} className="wtj-card" style={{background:'#1a1a1a',borderRadius:'12px',flex:'1 1 calc(50% - 10px)',padding:'20px',transition:'transform 0.3s ease'}}>
          <div className="wtj-icon" style={{marginBottom:'15px'}}>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">  
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z" fill="#ff6b00"/>
            </svg>
          </div>
          <h4 style={{color:'#fff',marginBottom:'10px',fontSize:'20px',fontWeight:'500'}}>{point.title}</h4>
          <p style={{color:'#aaa',fontSize:'16px'}}>{point.description}</p>
        </div>
      ))}
    </div>
...
<section className="pdc" style={{ backgroundColor: '#111827', color: '#fff', padding: '40px 20px' }}>
  <div className="container anim d0">
    <h2 className="pdc-head" style={{ fontFamily: 'Plus Jakarta Sans', fontSize: '32px', marginBottom: '20px', color: '#ff6b00' }}>
      Create High-Quality AI Images & Videos with ByteDance Generative Models
    </h2>
    <div className="pdc-grid" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
      {[{ 
        ...
        <div key={i} className={`pdc-card ${plan.highlighted ? 'featured' : ''}`} style={{ background: '#1a1a1a', borderRadius: '12px', padding: '20px', flex: '1 1 calc(50% - 20px)', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
          {plan.highlighted && <div className="pdc-badge" style={{ background: '#ff6b00', color: '#fff', padding: '5px 10px', borderRadius: '8px', display: 'inline-block', marginBottom: '10px' }}>Featured</div>}
          <h3 className="pdc-plan" style={{ fontFamily: 'Plus Jakarta Sans', fontSize: '24px', color: '#ff6b00', marginBottom: '10px' }}>{plan.name}</h3>
          <p className="pdc-price" style={{ fontFamily: 'Inter', fontSize: '18px', marginBottom: '10px' }}>
            <span className="pdc-amount">{plan.price || "Contact for Pricing"}</span>
            {plan.originalPrice && <span className="pdc-old" style={{ textDecoration: 'line-through', marginLeft: '10px', color: '#ccc' }}>{plan.originalPrice}</span>}
          </p>
          <span className="pdc-period" style={{ display: 'block', marginBottom: '15px', color: '#ff6b00' }}>{plan.period}</span>
          <p className="pdc-desc" style={{ fontFamily: 'Inter', fontSize: '16px', color: '#ddd', marginBottom: '15px' }}>{plan.description}</p>
          <ul className="pdc-list" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {plan.includes.map((include, index) => (
              <li key={index} className="pdc-check" style={{ display: 'flex', alignItems: 'center', marginBottom: '10px', color: '#ddd' }}>
                <svg width="16" height="16" fill="#ff6b00" style={{ marginRight: '10px' }}><path d="M14 0L4 10L0 6L1 5L4 8L13 0L14 0Z"/></svg>
                {include}
              </li>
            ))}
          </ul>
  ...
  <div style={{ position: 'relative', height: '300px', marginTop: '40px', borderRadius: '12px', overflow: 'hidden' }}>
    <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', position: 'absolute', inset: 0 }} />
  </div>
</section>

<section className="tcar" style={{background:'#111827',padding:'60px 0',color:'#ffffff'}}>
  <div className="container" style={{position:'relative'}}>
    <h2 className="tcar-head anim d0" style={{fontFamily:'Plus Jakarta Sans',fontSize:'clamp(32px, 5vw, 60px)',color:'#ffffff'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
    <p style={{fontFamily:'Inter',fontSize:'18px',marginBottom:'40px'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
    <div className="tcar-layout" style={{display:'flex',gap:'20px',position:'relative',marginBottom:'40px'}}>
      <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{width:'60%',borderRadius:'12px'}}/>
      <div className="tcar-rail" style={{flex:1,display:'grid',gap:'10px'}}>
        {[
          {"quote":"Seedream allows us to generate high-resolution creative visuals from simple prompts. It has dramatically reduced our design turnaround time.","author":"Vaishali Saxena","role":"Creative Director","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/04-6d75b26b02.webp"},
          {"quote":"Seedance’s ability to generate synchronized audio and video is incredibly powerful for storytelling and branded content.","author":"Vihaan Pandey","role":"Video Producer","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/07-82ab1c05ef.webp"},
          ...
          <div key={i} className="tcar-mini" style={{background:'#1a1a1a',padding:'15px',borderRadius:'8px',color:'#ffffff'}}>
            <p className="tcar-mini-stars" style={{marginBottom:'5px'}}>★★★★★</p>
            <p style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{t.quote}</p>
          </div>
        ))}
      </div>
    </div>
    <div className="tcar-stage anim-scale d1" style={{position:'relative',overflow:'hidden',height:'250px'}}>
      <div className="tcar-track" style={{display:'flex',transition:'transform 0.3s ease',transform:'translateX(0%)'}}>
        {[
          {"quote":"Seedream allows us to generate high-resolution creative visuals from simple prompts. It has dramatically reduced our design turnaround time.","author":"Vaishali Saxena","role":"Creative Director","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/04-6d75b26b02.webp"},
          {"quote":"Seedance’s ability to generate synchronized audio and video is incredibly powerful for storytelling and branded content.","author":"Vihaan Pandey","role":"Video Producer","avatar":"/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/07-82ab1c05ef.webp"},
          {"quote":"The multimodal editing capabilities in Seedream make it easy to refine images with precision.","author":"Anurag Malhotra","role":"Art Director","avatar":""},
          ...
          <div key={i} className="tcar-0" style={{flex:'0 0 100%',padding:'20px',background:'#1a1a1a',borderRadius:'12px',color:'#ffffff'}}>
            <p className="tcar-quote" style={{marginBottom:'10px',fontSize:'18px'}}>"{t.quote}"</p>
            <p className="tcar-stars" style={{marginBottom:'10px'}}>★★★★★</p>
            <div className="tcar-author" style={{display:'flex',alignItems:'center'}}>
              <img src={t.avatar || `https://via.placeholder.com/40?text=${t.author.charAt(0)}`} alt={t.author} style={{width:'40px',height:'40px',borderRadius:'50%',marginRight:'10px'}}/>
              <div className="tcar-text">
                <p className="tcar-name" style={{fontWeight:'bold',marginBottom:'5px'}}>{t.author}</p>
                <p className="tcar-role" style={{fontSize:'14px'}}>{t.role}</p>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="tcar-dots" style={{display:'flex',justifyContent:'center',marginTop:'20px'}}>
      {[0, 1, 2, 3, 4].map((index) => (
        <div key={index} className="tcar-dot" style={{width:'10px',height:'10px',borderRadius:'50%',background:index === 0 ? '#ff6b00' : '#555',margin:'0 5px'}}/>
      ))}
    </div>
  </div>
</section>

<section className="ctavb" style={{position: 'relative', overflow: 'hidden', background: '#0f172a', color: '#fff', padding: '60px 20px'}}>
  <div className="ctavb-bg" style={{position: 'relative', width: '100%', height: '100%', overflow: 'hidden'}}>
    <video autoPlay muted loop playsInline style={{position: 'absolute', width: '100%', height: '100%', objectFit: 'cover'}}>
      <source src="https://cdn.web.imagine.art/imagine-one/cdge/sora1.mp4" type="video/mp4" />
    </video>
    <div className="ctavb-overlay" style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0, 0, 0, 0.5)'}}>
      <img src="/output/generated-assets/seedream-4-5-and-seedance-1-5-pro/05-c4e1793d4f.webp" alt="Seedream 4.5 and Seedance 1.5 Pro" style={{width: '100%', height: '100%', objectFit: 'cover', display: 'block', position: 'absolute', inset: 0}} />
    </div>
  </div>
  <div className="container">
    <div className="ctavb-inner" style={{position: 'relative', zIndex: 1, maxWidth: '700px', margin: '0 auto', textAlign: 'center'}}>
      <h2 className="anim d0" style={{fontFamily: 'Plus Jakarta Sans', fontSize: 'clamp(32px, 5vw, 60px)', marginBottom: '20px', color: '#ff6b00'}}>Create High-Quality AI Images & Videos with ByteDance Generative Models</h2>
      <p className="anim d1" style={{fontFamily: 'Inter', fontSize: '18px', lineHeight: '1.6', color: '#ffffff', marginBottom: '40px'}}>Unlock the power of next-generation generative AI with Seedream 4.5 (Image Generation) and Seedance 1.5 Pro (Video Generation) - advanced foundation models developed by ByteDance for high-quality visual content creation.</p>
      <a href="#lead-form" className="ctavb-btn anim d2" style={{display: 'inline-block', padding: '15px 30px', background: '#ff6b00', color: '#fff', borderRadius: '8px', fontWeight: 'bold', textDecoration: 'none'}}>Generate with AI</a>
    </div>
  </div>
</section>
